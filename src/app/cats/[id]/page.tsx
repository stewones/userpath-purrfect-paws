...
export default function CatDetailPage() {
  ...
  useEffect(() => {
    const loadCat = async () => {
      ...
      try {
        const catData = await fetchCatById(params.id as string);
        if (catData) {
          setCat(catData);
        } else {
          // Cat not found, redirect to cats page
          router.push('/cats');
        }
      } catch (error) {
      ...
      <div className="bg-gray-50/90 min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="animate-pulse">
            <div className="h-8 w-24 bg-gray-200 rounded-2xl mb-8"></div>
